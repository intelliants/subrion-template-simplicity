{function name=menu pid=0}
    {if isset($data.$pid)}
        <ul class="nav {$class}{if $pid} menu_{$pid}{/if}{if in_array($position, ['left', 'right', 'user1', 'user2', 'top'])} flex-column{/if}">
            ...
            {foreach $data.$pid as $menu}
                ...
                {if in_array($position, ['left', 'right', 'user1', 'user2', 'top'])}
                    <li class="nav-item">
                        <a class="nav-link{if $menu.active} active{/if}" href="{if $menu.url}{$menu.url}{else}{$smarty.const.IA_SELF}#{/if}"{if $menu.nofollow} rel="nofollow"{/if}{if $menu.new_window} target="_blank"{/if}>{$menu.text|escape}</a>
                    </li>
                {else}
                    <li class="nav-item m_{$menu.page_name}
